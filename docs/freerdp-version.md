...
- `sdl3-freerdp.exe` — The SDL3-based Windows FreeRDP client with RAIL support
...
3. Extract `sdl3-freerdp.exe` and all `.dll` files
4. Replace contents of `installer/freerdp/` with the new files
5. Test a RemoteApp connection:
   ```
   sdl3-freerdp.exe /v:your-server /u:testuser /from-stdin /app:program:||YourApp /cert:tofu
   ```
...
- The launcher expects FreeRDP at `{app}\freerdp\sdl3-freerdp.exe`
...
| App doesn't float | Missing RAIL support | Ensure you're using `sdl3-freerdp.exe` (not the deprecated `wfreerdp.exe` or `xfreerdp`) |
